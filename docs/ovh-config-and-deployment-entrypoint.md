...
1. Prioritize authoritative docs under `docs/`, then use `docs/` for
   planned changes and `docs/` for recent evidence.
2. Treat `docs/archive/*` as historical context only.
3. If a conflict appears between docs, prefer `docs/` for current truth, `docs/`
   for active design direction, and `docs/` for the latest evidence.
