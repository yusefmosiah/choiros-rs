...
2. `docs/practice/` is the best default home for implemented or currently true
   system knowledge.
3. `docs/theory/` is for proposals, plans, explorations, and unfinished design.
4. `docs/state/` is for time-bound reports and snapshots.
...
- `docs/theory/`: thinking, proposals, draft decisions, prescriptive build
  guides, open notes
- `docs/practice/`: implemented or in-use knowledge, accepted decisions,
  operational guides, current contracts
- `docs/state/`: reports, checkpoints, handoffs, load tests, snapshots
...
- a draft decision starts in `docs/theory/decisions/` and moves to
  `docs/practice/decisions/` when accepted
- a build guide starts in `docs/theory/guides/` and becomes an operational
  guide in `docs/practice/guides/` when the system exists
- a transient test result lives in `docs/state/reports/`
...
2. Read the specific `docs/practice/` decisions or guides for the subsystem you
   care about.
3. Read `docs/theory/` only for future direction, open design, or unfinished
   migration context.
4. Read `docs/state/` for the latest evidence or checkpoint on a live thread.
...
If you truly do not know where something belongs yet, start in `docs/theory/`
or `docs/state/` depending on whether it is design intent or execution evidence.
...
sprawl. `docs/ATLAS.md` is the entrypoint. `docs/practice/` contains the best
current truth about implemented systems, `docs/theory/` contains proposals and
unfinished design, `docs/state/` contains time-bound execution evidence, and
...
1. `docs/theory/decisions/adr-0015-docs-kanban-architecture.md`
