  ...
  ... }:
{
  ...
  # Nix settings — binary cache configuration
  # cache.nixos.org is included by default but we list it explicitly so that
  # extra-substituters (FlakeHub) don't accidentally replace it.
  nix.settings = {
    ...
    # Download optimization: fetch from caches in parallel, fail fast on timeouts
    connect-timeout = 5;
    fallback = true;
  };
    ...
    serviceConfig = {
      ...
      ExecStart = pkgs.writeShellScript "cloud-hypervisor-start" ''
        ...
        if [[ "$BOOT_MODE" == "restore" ]] && [[ -d "$SNAPSHOT_DIR" ]] && [[ -f "$SNAPSHOT_DIR/state.json" ]]; then
          ...
          # ADR-0014 Phase 6: Apply vcpu/memory overrides from machine class config
          VCPU=$(cat "''${STATE_DIR}/machine-vcpu" 2>/dev/null || echo "")
          MEM_MB=$(cat "''${STATE_DIR}/machine-memory-mb" 2>/dev/null || echo "")
          if [[ -n "$VCPU" ]]; then
            ${pkgs.gnused}/bin/sed -i "s/boot=[0-9]*/boot=''${VCPU}/" \
              "''${STATE_DIR}/.microvm-run"
          fi
          if [[ -n "$MEM_MB" ]]; then
            ${pkgs.gnused}/bin/sed -i "s/size=[0-9]*M/size=''${MEM_MB}M/" \
              "''${STATE_DIR}/.microvm-run"
          fi
  ...
  # ADR-0014 Phase 6: Machine class configuration.
  # Nix generates this with resolved store paths for all available runners.
  # The hypervisor reads it on startup to map class names to runner paths.
  environment.etc."choiros/machine-classes.toml".text = let
    # Helper: only include a class if its runner was passed in
    mkClass = name: attrs:
      lib.optionalString (attrs.runner != null) ''
        [classes.${name}]
        hypervisor = "${attrs.hypervisor}"
        transport = "${attrs.transport}"
        vcpu = ${toString attrs.vcpu}
        memory_mb = ${toString attrs.memory_mb}
        runner = "${attrs.runner}"
        systemd_template = "${attrs.systemd_template}"
      '';
  in ''
    # Auto-generated by nix. Do not edit manually.

    # ── 2 vCPU / 1 GB (baseline) ──
    ${mkClass "ch-pmem-2c-1g" {
      hypervisor = "cloud-hypervisor"; transport = "pmem";
      vcpu = 2; memory_mb = 1024;
      runner = vmRunnerChPmem; systemd_template = "cloud-hypervisor";
    }}
    ${mkClass "ch-blk-2c-1g" {
      hypervisor = "cloud-hypervisor"; transport = "blk";
      vcpu = 2; memory_mb = 1024;
      runner = vmRunnerChBlk; systemd_template = "cloud-hypervisor";
    }}
    ${mkClass "fc-pmem-2c-1g" {
      hypervisor = "firecracker"; transport = "pmem";
      vcpu = 2; memory_mb = 1024;
      runner = vmRunnerFcPmem; systemd_template = "firecracker";
    }}
    ${mkClass "fc-blk-2c-1g" {
      hypervisor = "firecracker"; transport = "blk";
      vcpu = 2; memory_mb = 1024;
      runner = vmRunnerFcBlk; systemd_template = "firecracker";
    }}

    # ── 2 vCPU / 2 GB (standard workloads) ──
    ${mkClass "ch-pmem-2c-2g" {
      hypervisor = "cloud-hypervisor"; transport = "pmem";
  ...
  # Firecracker uses a JSON config file generated by microvm.nix.
  # The runner script handles config patching and invocation.
  systemd.services."firecracker@" = {
    description = "Firecracker VM for sandbox %i";
    requires = [ "tap-setup@%i.service" ];
    after = [ "tap-setup@%i.service" ];
    serviceConfig = {
      Type = "simple";
      ...
      TimeoutStopSec = 15;
      WorkingDirectory = "/opt/choiros/vms/state/%i";
      ExecStart = pkgs.writeShellScript "firecracker-start" ''
        set -euo pipefail
        INSTANCE="$1"
        STATE_DIR="/opt/choiros/vms/state/''${INSTANCE}"
        BOOT_MODE_FILE="''${STATE_DIR}/boot-mode"
        ...
        # Read machine class runner path
        RUNNER_DIR=""
        if [[ -f "''${STATE_DIR}/machine-runner" ]]; then
          RUNNER_DIR=$(cat "''${STATE_DIR}/machine-runner")
        fi
        if [[ -z "$RUNNER_DIR" ]]; then
          echo "ERROR: No machine-runner file found for ''${INSTANCE}"
          exit 1
        fi

        # ADR-0018: Read gateway token for injection
        GATEWAY_TOKEN=""
        if [[ -f "''${STATE_DIR}/gateway-token" ]]; then
          GATEWAY_TOKEN=$(cat "''${STATE_DIR}/gateway-token")
        fi
        ...
        if [[ -n "$GATEWAY_TOKEN" ]]; then
          ${pkgs.gnused}/bin/sed -i \
            "s|\"boot_args\": \"|\"boot_args\": \"choir.gateway_token=''${GATEWAY_TOKEN} |" \
            "''${STATE_DIR}/.microvm-run" 2>/dev/null || true
        fi

        # Apply vcpu/memory overrides from machine class config
        VCPU=$(cat "''${STATE_DIR}/machine-vcpu" 2>/dev/null || echo "")
        MEM_MB=$(cat "''${STATE_DIR}/machine-memory-mb" 2>/dev/null || echo "")
        if [[ -n "$VCPU" ]]; then
          ${pkgs.gnused}/bin/sed -i "s/\"vcpu_count\": [0-9]*/\"vcpu_count\": ''${VCPU}/" \
            "''${STATE_DIR}/.microvm-run" 2>/dev/null || true
        fi
        if [[ -n "$MEM_MB" ]]; then
          ${pkgs.gnused}/bin/sed -i "s/\"mem_size_mib\": [0-9]*/\"mem_size_mib\": ''${MEM_MB}/" \
            "''${STATE_DIR}/.microvm-run" 2>/dev/null || true
        fi

        chmod +x "''${STATE_DIR}/.microvm-run"
        echo "Cold booting Firecracker VM (MAC=''${VM_MAC} TAP=''${TAP})"
        exec "''${STATE_DIR}/.microvm-run"
      '' + " %i";
    };
  };
