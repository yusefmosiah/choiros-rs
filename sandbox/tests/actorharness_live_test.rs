//! ActorHarness live actor round-trip tests.
//!
//! Tests the production path that every eval/stub test bypasses:
//!
//!   `ActorRlmPort::spawn_subharness`
//!     → `Actor::spawn(SubharnessActor)`
//!     → `SubharnessMsg::Execute`
//!     → `run_subharness()` (calls AgentHarness with SubharnessAdapter)
//!     → emits `subharness.execute` + `subharness.result` events to EventStore
//!     → sends `ConductorMsg::SubharnessComplete` to conductor
...
//! ## Note on LLM calls
//!
//! `SubharnessActor` runs a real `AgentHarness` loop which requires an LLM.
...
use sandbox::actors::conductor::protocol::{ConductorMsg, SubharnessMsg, SubharnessResult};
use sandbox::actors::event_store::{
    AppendEvent, EventStoreActor, EventStoreArguments, EventStoreMsg,
};
use sandbox::actors::subharness::{SubharnessActor, SubharnessArguments};
...
// ─── Capture conductor: records SubharnessComplete/Failed messages ────────────
...
struct CapturingConductor {
    complete_tx: tokio::sync::mpsc::UnboundedSender<(String, SubharnessResult)>,
...
impl Actor for CapturingConductor {
    ...
    type State = (
        tokio::sync::mpsc::UnboundedSender<(String, SubharnessResult)>,
    ...
    async fn handle(
        ...
        match message {
            ConductorMsg::SubharnessComplete { correlation_id, result } => {
...
async fn make_capturing_conductor() -> (
    ...
    tokio::sync::mpsc::UnboundedReceiver<(String, SubharnessResult)>,
    ...
    let (actor, _) = Actor::spawn(
        None,
        CapturingConductor { complete_tx: complete_tx.clone(), failed_tx: failed_tx.clone() },
...
/// Spawning a `SubharnessActor` with a valid `SubharnessArguments` succeeds.
///
...
async fn test_subharness_actor_spawns_successfully() {
    ...
    let args = SubharnessArguments { event_store };
    ...
    assert!(result.is_ok(), "SubharnessActor must spawn without error");

    let (actor_ref, _handle) = result.unwrap();
    assert!(!actor_ref.get_id().to_string().is_empty(), "actor must have an ID");

    println!("  [SPAWN] SubharnessActor spawned, id: {}", actor_ref.get_id());
}

/// Two `SubharnessActors` can be spawned concurrently with different corr_ids.
...
async fn test_concurrent_subharness_actors_have_distinct_registry_names() {
    ...
    let (ref1, _h1) = Actor::spawn(
        ...
        SubharnessActor,
        SubharnessArguments { event_store: event_store.clone() },
    )
    .await
...
/// `ActorRlmPort::spawn_subharness` spawns a real `SubharnessActor`, sends it
/// `SubharnessMsg::Execute`, and the actor emits `subharness.execute` to EventStore.
///
...
/// `subharness.execute` event lands in EventStore via the actor's handler.
#[tokio::test]
async fn test_spawn_subharness_emits_execute_event_to_event_store() {
    let (event_store, _tmp) = make_event_store().await;
    ...
    // Spawn SubharnessActor directly (same as ActorRlmPort::spawn_subharness does)
    ...
    .await
    .expect("spawn SubharnessActor");

    // Send Execute message — this will invoke AgentHarness which needs LLM.
    // `subharness.execute` is emitted synchronously at the start of handle(),
    ...
    // Wait up to 500ms for `subharness.execute` event (emitted before LLM call).
    //
    // NOTE: `subharness.execute` uses `"correlation_id"` as the payload key (not `"corr_id"`),
    // so `GetEventsByCorrId` (which searches for `"corr_id":"..."`) will NOT find it.
    // We use `GetRecentEvents` filtered by `actor_id = "subharness:{corr_id}"` instead,
    // since `emit_subharness_execute` sets `actor_id = format!("subharness:{}", correlation_id)`.
    //
    // This also documents the mismatch: `subharness.execute` payload should include
    // `"corr_id"` alongside `"correlation_id"` so it is queryable via `GetEventsByCorrId`.
    let deadline = std::time::Instant::now() + std::time::Duration::from_millis(3000);
    let actor_id_filter = format!("subharness:{corr_id}");
    let mut found = false;
    while std::time::Instant::now() < deadline {
        let events = ractor::call_t!(
            ...
            |reply| EventStoreMsg::GetRecentEvents {
                since_seq: 0,
                limit: 50,
                event_type_prefix: Some("subharness.execute".to_string()),
        ...
        if !events.is_empty() {
            ...
            assert_eq!(
                ev_corr, corr_id,
                "subharness.execute event must have correct correlation_id"
            );
            println!(
                "  [EXECUTE-EVENT] subharness.execute confirmed in EventStore, corr_id: {corr_id}"
            );
            break;
        }
    ...
    assert!(
        found,
        "subharness.execute event must land in EventStore within 3s of Execute message"
    );

    // Drain the conductor channels (SubharnessComplete or SubharnessFailed may arrive
...
/// `ActorRlmPort::spawn_subharness` uses the correct registry name format.
///
/// Verifies that `ActorRlmPort::spawn_subharness` spawns with the name
/// `subharness-{corr_id}` so that duplicate corr_ids are caught at spawn time
/// (registry collision → spawn error).
#[tokio::test]
async fn test_spawn_subharness_via_alm_port_uses_correct_registry_name() {
    let (event_store, _tmp) = make_event_store().await;
    ...
    // Call spawn_subharness — this will spawn `SubharnessActor` with name
    ...
    // verify is that `subharness.execute` event landed, proving the message was dispatched.
    let events = ractor::call_t!(
        event_store,
    ...
    println!(
        "  [PORT-SPAWN] spawn_subharness completed, execute events found: {}",
        events.len()
    );
}

/// Duplicate corr_id spawn: second `spawn_subharness` with the same corr_id
...
async fn test_duplicate_corr_id_spawn_does_not_panic() {
    ...
    // Second spawn with the same registry name: should fail, but ActorRlmPort
    ...
    port.spawn_subharness("dup test", serde_json::json!({}), &corr_id).await;
...
/// Full round-trip: SubharnessActor receives Execute, runs AgentHarness with a
/// simple objective, emits `subharness.result` to EventStore, sends
/// `SubharnessComplete` to conductor, and `resolve_source` returns the output.
///
...
async fn test_full_subharness_round_trip_via_alm_port() {
    ...
    // Wait for SubharnessComplete (or Failed) — up to 60s for LLM
    ...
    tokio::select! {
        Ok(Some((received_corr, result))) = tokio::time::timeout(timeout, complete_rx.recv()) => {
            ...
            println!(
                "  [COMPLETE] SubharnessComplete: corr:{} steps:{} satisfied:{}",
        ...
        Ok(Some((received_corr, reason))) = tokio::time::timeout(timeout, failed_rx.recv()) => {
            panic!("SubharnessActor failed for corr:{received_corr}: {reason}");
        }
        _ = tokio::time::sleep(timeout) => {
            panic!("Timeout waiting for SubharnessComplete after 60s");
        }
    }

    // Verify `subharness.result` event landed in EventStore
    let events = ractor::call_t!(
        event_store,
        |reply| EventStoreMsg::GetEventsByCorrId {
            corr_id: corr_id.clone(),
            event_type_prefix: Some("subharness.result".to_string()),
            reply,
        },
        2000
    )
    ...
    assert!(
        !events.is_empty(),
        "subharness.result event must land in EventStore after completion"
    );
    ...
    assert!(
        result_text.is_some(),
        "resolve_source must return Some after subharness.result event lands"
    );
    let text = result_text.unwrap();
    assert!(!text.is_empty(), "result text must not be empty");

    println!("  [RESOLVE] resolve_source returned: '{}'", &text[..80.min(text.len())]);
}

/// SubharnessComplete is correctly routed back through the CapturingConductor.
///
/// Tests the `ConductorMsg::SubharnessComplete` path directly, without LLM,
/// by manually emitting the message as the SubharnessActor would.
#[tokio::test]
async fn test_subharness_complete_message_routed_to_conductor() {
    ...
    // Send SubharnessComplete directly — simulates what SubharnessActor does
    conductor
        .send_message(ConductorMsg::SubharnessComplete {
            correlation_id: corr_id.clone(),
            result: result.clone(),
        })
        .expect("send SubharnessComplete");
    ...
    .await
    .expect("timeout waiting for SubharnessComplete")
    ...
    println!("  [ROUTE] SubharnessComplete routed correctly to conductor");
}

/// `SubharnessResult` in EventStore is queryable via `GetEventsByCorrId`.
///
...
async fn test_subharness_result_event_queryable_by_corr_id() {
    ...
    // Write a subharness.result event directly (as SubharnessActor does)
    ...
    assert!(!events.is_empty(), "subharness.result event must be queryable by corr_id");
    ...
    // Verify that `resolve_source` finds it via the real ActorRlmPort logic
    let conductor = make_capturing_conductor().await.0;
    let port = ActorRlmPort::new(
    ...
    assert!(
        result.is_some(),
        "resolve_source must find subharness.result by corr_id"
    );
    ...
    println!("  [QUERY] subharness.result queryable and resolve_source returns correct text");
}
