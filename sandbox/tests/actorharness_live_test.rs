...
//!
//!   `ActorAlmPort::spawn_actor_harness`
//!     → `Actor::spawn(ActorHarnessActor)`
//!     → `ActorHarnessMsg::Execute`
//!     → `run_actor_harness()` (calls AgentHarness with ActorHarnessAdapter)
//!     → emits `actor_harness.execute` + `actor_harness.result` events to EventStore
//!     → sends `ConductorMsg::ActorHarnessComplete` to conductor
...
//! ## Note on LLM calls
//!
//! `ActorHarnessActor` runs a real `AgentHarness` loop which requires an LLM.
...
use sandbox::actors::conductor::protocol::{ConductorMsg, ActorHarnessMsg, ActorHarnessResult};
use sandbox::actors::event_store::{
    AppendEvent, EventStoreActor, EventStoreArguments, EventStoreMsg,
};
use sandbox::actors::actor_harness_actor::{ActorHarnessActor, ActorHarnessArguments};
...
// ─── Capture conductor: records ActorHarnessComplete/Failed messages ────────────
...
struct CapturingConductor {
    complete_tx: tokio::sync::mpsc::UnboundedSender<(String, ActorHarnessResult)>,
...
impl Actor for CapturingConductor {
    ...
    type State = (
        tokio::sync::mpsc::UnboundedSender<(String, ActorHarnessResult)>,
...
async fn make_capturing_conductor() -> (
    ...
    tokio::sync::mpsc::UnboundedReceiver<(String, ActorHarnessResult)>,
...
/// Spawning a `ActorHarnessActor` with a valid `ActorHarnessArguments` succeeds.
///
...
async fn test_subharness_actor_spawns_successfully() {
    ...
    let args = ActorHarnessArguments { event_store };
    let result = Actor::spawn(Some(format!("subharness-{corr_id}")), ActorHarnessActor, args).await;

    assert!(result.is_ok(), "ActorHarnessActor must spawn without error");
    ...
    println!(
        "  [SPAWN] ActorHarnessActor spawned, id: {}",
        actor_ref.get_id()
    );
}

/// Two `ActorHarnessActors` can be spawned concurrently with different corr_ids.
...
/// `ActorAlmPort::spawn_actor_harness` spawns a real `ActorHarnessActor`, sends it
/// `ActorHarnessMsg::Execute`, and the actor emits `actor_harness.execute` to EventStore.
///
...
/// `actor_harness.execute` event lands in EventStore via the actor's handler.
#[tokio::test]
async fn test_spawn_actor_harness_emits_execute_event_to_event_store() {
    let (event_store, _tmp) = make_event_store().await;
    ...
    // Spawn ActorHarnessActor directly (same as ActorAlmPort::spawn_actor_harness does)
    ...
    .await
    .expect("spawn ActorHarnessActor");

    // Send Execute message — this will invoke AgentHarness which needs LLM.
    // `actor_harness.execute` is emitted synchronously at the start of handle(),
    ...
    // Wait up to 500ms for `actor_harness.execute` event (emitted before LLM call).
    //
    // NOTE: `actor_harness.execute` uses `"correlation_id"` as the payload key (not `"corr_id"`),
    // so `GetEventsByCorrId` (which searches for `"corr_id":"..."`) will NOT find it.
    // We use `GetRecentEvents` filtered by `actor_id = "actor_harness:{corr_id}"` instead,
    // since `emit_actor_harness_execute` sets `actor_id = format!("actor_harness:{}", correlation_id)`.
    //
    // This also documents the mismatch: `actor_harness.execute` payload should include
    // `"corr_id"` alongside `"correlation_id"` so it is queryable via `GetEventsByCorrId`.
    let deadline = std::time::Instant::now() + std::time::Duration::from_millis(3000);
    let actor_id_filter = format!("actor_harness:{corr_id}");
    let mut found = false;
    while std::time::Instant::now() < deadline {
        let events = ractor::call_t!(
            ...
            |reply| EventStoreMsg::GetRecentEvents {
                since_seq: 0,
                limit: 50,
                event_type_prefix: Some("actor_harness.execute".to_string()),
        ...
        if !events.is_empty() {
            ...
            assert_eq!(
                ev_corr, corr_id,
                "actor_harness.execute event must have correct correlation_id"
            );
            println!(
                "  [EXECUTE-EVENT] actor_harness.execute confirmed in EventStore, corr_id: {corr_id}"
            );
            break;
        }
    ...
    assert!(
        found,
        "actor_harness.execute event must land in EventStore within 3s of Execute message"
    );

    // Drain the conductor channels (ActorHarnessComplete or ActorHarnessFailed may arrive
...
/// `ActorAlmPort::spawn_actor_harness` uses the correct registry name format.
///
/// Verifies that `ActorAlmPort::spawn_actor_harness` spawns with the name
/// `subharness-{corr_id}` so that duplicate corr_ids are caught at spawn time
/// (registry collision → spawn error).
#[tokio::test]
async fn test_spawn_actor_harness_via_alm_port_uses_correct_registry_name() {
    let (event_store, _tmp) = make_event_store().await;
    ...
    // Call spawn_actor_harness — this will spawn `ActorHarnessActor` with name
    // `subharness-{corr_id}` then send Execute.
    port.spawn_actor_harness(
    ...
    // verify is that `actor_harness.execute` event landed, proving the message was dispatched.
    let events = ractor::call_t!(
        event_store,
    ...
    println!(
        "  [PORT-SPAWN] spawn_actor_harness completed, execute events found: {}",
        events.len()
    );
}

/// Duplicate corr_id spawn: second `spawn_actor_harness` with the same corr_id
...
async fn test_duplicate_corr_id_spawn_does_not_panic() {
    ...
    port.spawn_actor_harness("dup test", serde_json::json!({}), &corr_id)
        .await;
...
/// Full round-trip: ActorHarnessActor receives Execute, runs AgentHarness with a
/// simple objective, emits `actor_harness.result` to EventStore, sends
/// `ActorHarnessComplete` to conductor, and `resolve_source` returns the output.
///
...
async fn test_full_subharness_round_trip_via_alm_port() {
    ...
    // Wait for ActorHarnessComplete (or Failed) — up to 60s for LLM
    ...
    tokio::select! {
        Ok(Some((received_corr, result))) = tokio::time::timeout(timeout, complete_rx.recv()) => {
            ...
            println!(
                "  [COMPLETE] ActorHarnessComplete: corr:{} steps:{} satisfied:{}",
        ...
        Ok(Some((received_corr, reason))) = tokio::time::timeout(timeout, failed_rx.recv()) => {
            panic!("ActorHarnessActor failed for corr:{received_corr}: {reason}");
        }
        _ = tokio::time::sleep(timeout) => {
            panic!("Timeout waiting for ActorHarnessComplete after 60s");
        }
    }

    // Verify `actor_harness.result` event landed in EventStore
    let events = ractor::call_t!(
        event_store,
        |reply| EventStoreMsg::GetEventsByCorrId {
            corr_id: corr_id.clone(),
            event_type_prefix: Some("actor_harness.result".to_string()),
            reply,
        },
        2000
    )
    ...
    assert!(
        !events.is_empty(),
        "actor_harness.result event must land in EventStore after completion"
    );
    ...
    assert!(
        result_text.is_some(),
        "resolve_source must return Some after actor_harness.result event lands"
    );
...
/// ActorHarnessComplete is correctly routed back through the CapturingConductor.
///
/// Tests the `ConductorMsg::ActorHarnessComplete` path directly, without LLM,
/// by manually emitting the message as the ActorHarnessActor would.
#[tokio::test]
async fn test_actor_harness_complete_message_routed_to_conductor() {
    ...
    // Send ActorHarnessComplete directly — simulates what ActorHarnessActor does
    conductor
        .send_message(ConductorMsg::ActorHarnessComplete {
            correlation_id: corr_id.clone(),
            result: result.clone(),
        })
        .expect("send ActorHarnessComplete");

    // Should arrive within 100ms
    let received = tokio::time::timeout(std::time::Duration::from_millis(100), complete_rx.recv())
        .await
        .expect("timeout waiting for ActorHarnessComplete")
    ...
    println!("  [ROUTE] ActorHarnessComplete routed correctly to conductor");
}

/// `ActorHarnessResult` in EventStore is queryable via `GetEventsByCorrId`.
///
...
async fn test_subharness_result_event_queryable_by_corr_id() {
    ...
    // Write a actor_harness.result event directly (as ActorHarnessActor does)
    ...
    assert!(
        !events.is_empty(),
        "actor_harness.result event must be queryable by corr_id"
    );
    ...
    assert!(
        result.is_some(),
        "resolve_source must find actor_harness.result by corr_id"
    );
    ...
    println!("  [QUERY] actor_harness.result queryable and resolve_source returns correct text");
}
