...
impl ToolRegistry {
    ...
    pub fn new() -> Self {
        ...
        tools.insert("write_file".to_string(), Box::new(WriteFileTool) as Box<dyn Tool>);
        tools.insert("list_files".to_string(), Box::new(ListFilesTool) as Box<dyn Tool>);
        tools.insert("search_files".to_string(), Box::new(SearchFilesTool) as Box<dyn Tool>);
...
impl Tool for BashTool {
    ...
    fn execute(&self, args: Value) -> Result<ToolOutput, ToolError> {
        ...
        match output {
            ...
            Err(_) => Err(ToolError::new(format!("Command timed out after {}ms", timeout_ms))),
        }
    }
}
...
impl Tool for ReadFileTool {
    ...
    fn execute(&self, args: Value) -> Result<ToolOutput, ToolError> {
        ...
        if path_buf.is_absolute() && !path_buf.starts_with("/Users/wiz/choiros-rs") {
            return Err(ToolError::new("Cannot read files outside project directory"));
        }
...
impl Tool for WriteFileTool {
    ...
    fn execute(&self, args: Value) -> Result<ToolOutput, ToolError> {
        ...
        let path_buf = std::path::PathBuf::from(path);
        if path_buf.is_absolute() && !path_buf.starts_with("/Users/wiz/choiros-rs") {
            return Err(ToolError::new("Cannot write files outside project directory"));
        }
...
impl Tool for ListFilesTool {
    ...
    fn execute(&self, args: Value) -> Result<ToolOutput, ToolError> {
        ...
        // Security check
        if path_buf.is_absolute() && !path_buf.starts_with("/Users/wiz/choiros-rs") {
            return Err(ToolError::new("Cannot list files outside project directory"));
        }
    ...
    fn execute(&self, args: Value) -> Result<ToolOutput, ToolError> {
        ...
        let output = runtime.block_on(async {
            tokio::process::Command::new("rg")
                .arg("-n")
                .arg("--type-add")
                .arg(format!("custom:*.{}", file_pattern.trim_start_matches("*").trim_start_matches(".")))
                .arg("-tcustom")
                .arg(pattern)
                .arg(path)
                .output()
