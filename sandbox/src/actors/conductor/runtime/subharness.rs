...
//!
//! Wires `ConductorMsg::SubharnessComplete`, `SubharnessFailed`, and
//! `SubharnessProgress` into the existing capability call / run
...
use crate::actors::conductor::protocol::SubharnessResult;
...
impl ConductorActor {
    ...
    pub(crate) async fn handle_subharness_complete(
        &self,
        ...
        let run_id = match state.tasks.get_run_id_for_call(&correlation_id) {
            ...
            None => {
                tracing::warn!(
                    correlation_id = %correlation_id,
                    "SubharnessComplete: no matching capability call found; ignoring"
                );
                return Ok(());
            }
        };
        ...
        self.finalize_run_if_quiescent_subharness(state, &run_id)
            .await?;
        Ok(())
    }

    /// Handle subharness failure.
    pub(crate) async fn handle_subharness_failed(
        &self,
        state: &mut ConductorState,
        correlation_id: String,
        reason: String,
        ...
        let run_id = match state.tasks.get_run_id_for_call(&correlation_id) {
            ...
            None => {
                tracing::warn!(
                    correlation_id = %correlation_id,
                    "SubharnessFailed: no matching capability call found; ignoring"
                );
                return Ok(());
            }
        };
    ...
    /// Handle in-flight progress from a running SubharnessActor.
    ///
    ...
    pub(crate) async fn handle_subharness_progress(
        &self,
        ...
        let _ = state.event_store.send_message(EventStoreMsg::AppendAsync {
            event: AppendEvent {
                event_type: "subharness.progress.received".to_string(),
    ...
    async fn finalize_run_if_quiescent_subharness(
        &self,
        state: &mut ConductorState,
        run_id: &str,
    ) -> Result<(), ActorProcessingErr> {
