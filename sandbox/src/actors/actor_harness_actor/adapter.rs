...
impl ActorHarnessAdapter {
    ...
    async fn execute_bash(&self, command: &str, timeout_ms: u64) -> Result<(String, i32), String> {
...
"#
    ...
    fn get_system_context(&self, ctx: &ExecutionContext) -> String {
        let context_summary = if self.context.is_null()
            || self.context == serde_json::Value::Object(serde_json::Map::new())
        {
            String::new()
        } else {
            format!(
